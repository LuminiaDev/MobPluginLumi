...
public abstract class FlyingMonster extends FlyingEntity implements Monster {
    ...
    public float getMinDamage(Integer difficulty) {
        if (difficulty == null || difficulty > 3 || difficulty < 0) {
            difficulty = Server.getInstance().getDifficulty().getId();
        }
    ...
    public void setDamage(float damage) {
        this.setDamage(damage, Server.getInstance().getDifficulty().getId());
    }
    ...
    public void setMinDamage(float damage) {
        this.setMinDamage(damage, Server.getInstance().getDifficulty().getId());
    }
    ...
    public void setMaxDamage(float damage) {
        this.setMaxDamage(damage, Server.getInstance().getDifficulty().getId());
    }
    ...
    public boolean onUpdate(int currentTick) {
        if (this.server.getDifficulty().getId() < 1) {
            this.close();
            return false;
        }
